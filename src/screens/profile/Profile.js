  ...
  render() {
    ...
    return (
      <View style={gStyles.container}>
        <StackHeader leftTitle="完善资料" rightTitle="保存" onPressLeft={() => this.props.navigation.goBack()} />
        <FlatList
          style={{ marginTop: 45 }}
          data={options}
          renderItem={({ item, index }) => {
            return (
              <TouchableHighlight onPress={() => this.toChangePage(item.key)} underlayColor={'#cccccc'}>
                <ListItem
                  title={item.title}
